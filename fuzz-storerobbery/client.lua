...
local function Alert(id)
    ...
    cfg.alerted = true
    TriggerServerEvent("fuzz-storerobbery:server:registerAlert", id, true)
...
local function RobRegistar()
    ...
    TriggerServerEvent("fuzz-storerobbery:server:setUse", CurrentStore, RegZoneID, true)
    ...
    local success = exports['ran-minigames']:MineSweep(prize, 8, 3, "left")
    if success then
    ...
    TriggerServerEvent("fuzz-storerobbery:server:setUse", CurrentStore, RegZoneID, false)
end
    ...
    if searchLoc.iscomputer then
        ...
        if success then
            ...
            if canGet then
                ...
                SetTimeout(1000 * TimeToWait, function()
                    lib.callback.await("fuzz-storerobbery:server:combination", false, storeid, sid, true)
                    QBCore.Functions.Notify("You got the combination key")
                end)
            else
                lib.callback.await("fuzz-storerobbery:server:combination", false, storeid, sid, false)
        ...
        QBCore.Functions.Progressbar('search-combination', 'Searching for combination', 5000, false, true,
            { -- Name | Label | Time | useWhileDead | canCancel
                ...
                lib.callback.await("fuzz-storerobbery:server:combination", false, storeid, sid, canGet)
                if canGet then
                    QBCore.Functions.Notify("You got the combination key")
                else
...
local function SetupStore(id)
    ...
    local function Hack(self)
        ...
        if cfg.cooldown then return end
        local success = exports['ran-minigames']:MemoryCard(Config.MemoryGameTime)
        TriggerServerEvent("fuzz-storerobbery:server:setHackUse", id, true)
        if success then
            ...
            if chance <= Config.ShopKeeperChance  then
            ...
            PissedGuy = CreatePed(26, current, x, y, z-1, 90.0, true, true)
            NetworkRegisterEntityAsNetworked(PissedGuy)
            networkID = NetworkGetNetworkIdFromEntity(PissedGuy)
            SetNetworkIdCanMigrate(networkID, true)
            SetNetworkIdExistsOnAllMachines(networkID, true)
            SetPedRandomComponentVariation(PissedGuy)
            SetPedRandomProps(PissedGuy)
            SetEntityAsMissionEntity(PissedGuy)
            SetEntityVisible(PissedGuy, true)
            SetPedRelationshipGroupHash(PissedGuy)
            SetPedAccuracy(PissedGuy)
            SetPedArmour(PissedGuy)
            SetPedCanSwitchWeapon(PissedGuy, true)
            SetPedDropsWeaponsWhenDead(PissedGuy,false)
            SetPedFleeAttributes(PissedGuy, false)
            GiveWeaponToPed(PissedGuy, Config.Weapon, 1, false, true)
            TaskCombatPed(PissedGuy, PlayerPedId(), 0, 16)
            SetPedCombatAttributes(PissedGuy, 46, true)
            TriggerEvent('animations:client:EmoteCommandStart', {"c"}) 
        end
        TriggerServerEvent("fuzz-storerobbery:server:setHackUse", id, false)
        end
    end
    local function OpenPinPrompt()
        ...
        if num == cfg.combination then
            lib.callback.await("fuzz-storerobbery:server:setSafeState", false, CurrentStore, true)
...
lib.callback.register("fuzz-storerobbery:client:openConfigContext", function()
    return ConfigContext()
end)

lib.callback.register("fuzz-storerobbery:client:resetStore", function()
    if not CurrentStore then return end
    local cfg = Config.Store[CurrentStore]
    if not cfg then return end
...
RegisterNetEvent("fuzz-storerobbery:client:setConfigs", function(cfg)
    Config.Store = cfg
end)

RegisterNetEvent("fuzz-storerobbery:client:setStoreConfig", function(id, cfg)
